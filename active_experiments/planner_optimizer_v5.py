"""Planner agent optimizer: Gemini-driven ``execute_plan`` generation.

Run from the ``finance-ai-penny`` repository root with ``GEMINI_API_KEY`` set (e.g. in ``.env``)::

    python active_experiments/planner_optimizer_v5.py --test hows_my_accounts_doing
    python active_experiments/planner_optimizer_v5.py --test app_usage_where_net_worth
    python active_experiments/planner_optimizer_v5.py --test all
    python active_experiments/planner_optimizer_v5.py --batch 6
    python active_experiments/planner_optimizer_v5.py --test 0 --no-thinking
"""
...
**0. Use `app_usage_info` for Hey Penny product and UI questions (no personal financial data):**
- When the Last User Request is about **how the Hey Penny app works**, **where to find a screen or feature**, **navigation** (tabs, sections, buttons), **what categories exist or how Penny would label something**, **Split It Up**, goals UI, or **product limits** (e.g. linked accounts only, no custom categories)—call **`app_usage_info`** alone. It returns dense factual answers using real in-app labels; it does **not** read the user's accounts, transactions, or balances.
- **Do not** call `lookup_user_accounts_transactions_income_and_spending_patterns` for these pure app/help questions (e.g. "Where can I see my net worth?" is navigation → `app_usage_info`; "What's my net worth?" is **their data** → lookup first).
- If a request mixes app help with a need for the user's numbers, call `lookup` (and/or other skills) as needed for data, and use `app_usage_info` only when a separate app-explanation step is still required—or fold both into one plan if one skill suffices.
...
**5. Requests Not Supported by Available Skills:** The available skills support: (1) looking up user accounts/transactions/income/spending/subscriptions, (2) creating spending limits or income/savings goals, (3) researching and strategizing financial outcomes, (4) updating transaction categories or creating category rules, (5) answering Hey Penny **app usage** questions (navigation, categories, how features work—via `app_usage_info`). If the Last User Request clearly falls outside these, do not call any skills. Return `(True, brief polite message)` (one or two sentences) that you cannot do that and what you can help with.
...
- All **skill functions** return a `tuple[bool, str]`.
    ...
    - **CRITICAL**: For all skill functions, ensure that the request parameters (e.g., `lookup_request`, `creation_request`, `strategize_request`, `categorize_request`) effectively incorporate relevant information from the `Previous Conversation` and, when available, the `input_info` to accurately address the `Last User Request`. For `app_usage_info`, only `usage_request` is used: merge any needed prior-step or conversation context **into** `usage_request` text (do not rely on `input_info` for that skill).
...
- `app_usage_info(usage_request: str) -> tuple[bool, str]`
  - Answers questions about the **Hey Penny** app itself: navigation (which tab/section/button to use), category definitions and how Penny labels spending, how features work (e.g. **Split It Up**, goals), and product constraints (e.g. linked accounts only, fixed category list). Output is factual plain text with real in-app labels—not the user's balances or transactions. **This skill does not take `input_info`:** if a prior step produced context the answer should reflect, write that context into `usage_request` itself.
  - Use this instead of lookup when the user is not asking for *their* financial data, only how to use the app or what the app supports.
...
output:"""
    ...
    if code:
        ...
        try:
            ...
            def wrapped_app_usage(*args, **kwargs):
                print("\n[FUNCTION CALL] app_usage_info")
                print(f"  args: {args}")
                out = app_usage_info(*args, **kwargs)
                print(f"  [RETURN] success: {out[0]}")
                print(f"  [RETURN] output: {out[1][:500]}{'...' if len(out[1]) > 500 else ''}")
                return out
            ...
            namespace = {
                ...
                "app_usage_info": wrapped_app_usage,
            }
            exec(code, namespace)
...
def parse_planner_test_input(raw: str) -> tuple[str, str]:
    """Split a static test *input* string into last user request and previous conversation."""
    s = raw.strip()
...
# Batches: 1 lookup/strategize, 2 budgets/goals, 3 unsupported reminders, 4 categorize,
# 5 acknowledgments and edges, 6 app usage (Hey Penny UI).
TEST_CASES = [
    {
...
Assistant: The trip definitely added up! Over the past two weeks, you spent $1,890 on travel and hotels, plus $520 on restaurants and dining out.""",
        "output": """Expected: lookup_user_accounts_transactions_income_and_spending_patterns only (general question). Return (success, lookup_result). No strategize.""",
    },
    {
        "name": "how_is_my_net_worth_doing_lately",
        "batch": 1,
        "input": """**Last User Request**: how is my net worth doing lately?
...
Assistant: I don't have access to your credit score, but I can help you track your spending patterns that affect it.""",
        "output": """Expected: lookup only for net worth summary. Return (success, lookup_result). No strategize (informational).""",
    },
    {
        "name": "research_and_strategize_savings_plan",
        "batch": 1,
        "input": """**Last User Request**: I want to save $10,000 for a down payment on a house. What's the best plan to get there?
...
Assistant: Your total monthly spending averages around $3,200, and your monthly income is about $4,500.""",
        "output": """Expected: lookup first (income/spending), then research_and_strategize_financial_outcomes for savings plan. Return (success, strategy_result). No create_budget unless user asks to create.""",
    },
    {
        "name": "research_and_strategize_vacation_affordability",
        "batch": 1,
        "input": """**Last User Request**: Is it feasible for me to take a 2-week vacation to Japan next year? Research the costs and tell me if I can afford it.
...
Assistant: Based on your recent spending patterns, you're saving approximately $800 per month.""",
        "output": """Expected: lookup (current finances) then research (Japan trip costs / feasibility). Return (success, strategy_result or combined message).""",
    },
    {
        "name": "save_5000_in_6_months",
        "batch": 1,
        "input": """**Last User Request**: I want to save $5000 in the next 6 months.
...
Assistant: Your total monthly spending averages around $3,200, and your monthly income is about $4,500.""",
        "output": """Expected: lookup first, then research for plan. Optionally create_budget_or_goal only if user intent is to create a goal (savings goal is explicit).""",
    },
    {
...
""",
        "output": """Expected: create_budget_or_goal(creation_request='...food budget $500 next month...'). Optionally lookup first for context.""",
    },
    {
...
""",
        "output": """Expected: lookup (accounts) then create_budget_or_goal for savings goal. Optionally research for plan.""",
    },
    {
...
""",
        "output": """Expected: No create_budget_or_goal call (request not supported). execute_plan returns (True, brief message that this cannot be done; can help with budgets/goals instead).""",
    },
    {
        "name": "reminder_cancel_netflix_november_30",
        "batch": 3,
        "input": """**Last User Request**: remind me to cancel Netflix on November 30th.
...
""",
    ...
    },
    {
        ...
        "batch": 3,
        "input": """**Last User Request**: notify me when my checking account balance drops below $1000.
...
""",
    ...
    },
    {
        ...
        "batch": 3,
        "input": """**Last User Request**: notify me when my savings account balance drops below $1000.
...
""",
    ...
    },
    {
        ...
        "batch": 4,
        "input": """**Last User Request**: that's for my disneyland trip for next month. categorize it as travel.

**Previous Conversation**:

Assistant: There's an uncategorized $525 transaction.""",
        "output": """Expected: update_transaction_category_or_create_category_rules(categorize_request='...$525 transaction...travel...Disneyland...'). Optionally lookup to find transaction. No add_to_memory (not in P:AgentPlanner skills).""",
    },
    {
        "name": "thanks_acknowledgment",
        "batch": 5,
        "input": """**Last User Request**: thanks
...
I see 24 more forecasts too!""",
        "output": """Expected: No skill calls (acknowledgment only). execute_plan returns (True, a brief polite acknowledgment). Plan should not call lookup/create/research/categorize/app_usage_info.""",
    },
    {
        "name": "ok_yes_to_penny_question",
        "batch": 5,
        "input": """**Last User Request**: ok

**Previous Conversation**:

User: set a savings goal of $2000 by end of the month
Penny: I can create a savings goal of $2000 by end of the month. Should I go ahead and create that for you?""",
        "output": """Expected: 'ok' means yes to Penny's question. execute_plan should call create_budget_or_goal(creation_request='...savings goal $2000 by end of month...'). Do NOT return (True, "You're welcome!") or treat as acknowledgment.""",
    },
    {
...
Penny: You spent $412 on dining out last month—about $85 over your usual.""",
        "output": """Expected: 'ok' means 'I see' or 'got it' (acknowledging the answer). No skill calls. execute_plan returns (True, brief natural acknowledgment). Do NOT return 'You're welcome!' (user did not thank). Do NOT call lookup/create/research/categorize/app_usage_info.""",
    },
    {
        "name": "unintelligible_request",
        "batch": 5,
        "input": """**Last User Request**: asdfgh qwerty zxcv ???

**Previous Conversation**:


""",
        "output": """Expected: Last User Request is unintelligible. No skill calls. execute_plan returns (True, brief clarification asking what the user meant). Do NOT call lookup/create/research/categorize.""",
    },
    {
        "name": "app_usage_where_net_worth",
        "batch": 6,
        "input": """**Last User Request**: Where can I see my net worth?

**Previous Conversation**:


""",
        "output": """Expected: app_usage_info only (navigation / Account tab / Net Worth). No lookup (user asked where in the app, not for their number). Return (True, reply from app_usage_info).""",
    },
    {
        "name": "app_usage_manual_transaction",
        "batch": 6,
        "input": """**Last User Request**: How do I manually add a transaction?

**Previous Conversation**:


""",
        "output": """Expected: app_usage_info only—explain linked accounts / no manual add. No lookup.""",
    },
    {
        "name": "app_usage_change_category_paths",
        "batch": 6,
        "input": """**Last User Request**: What are the different ways to access the page that allows me to change a transaction's category?

**Previous Conversation**:


""",
        "output": """Expected: app_usage_info only—paths to Each Transaction's Category (Home, Income vs. Spending, Account, etc.). No lookup.""",
    },
    {
        "name": "app_usage_custom_category",
        "batch": 6,
        "input": """**Last User Request**: Can I create a custom category for my 'Hobby' expenses?

**Previous Conversation**:


""",
        "output": """Expected: app_usage_info only—no custom categories; map to real subcategory (e.g. Entertainment). No lookup.""",
    },
    {
        "name": "mixed_lookup_and_app_usage",
        "batch": 7,
        "input": """**Last User Request**: What is my net worth? How do I manually add a transaction?

**Previous Conversation**:


""",
        "output": """Expected: Lookup and app_usage_info calls; Return lookup result and app_usage_info result.""",
    },
]
...
def indices_for_batch(batch_num: int) -> list[int]:
    return [i for i, tc in enumerate(TEST_CASES) if tc["batch"] == batch_num]
...
def print_available_test_cases():
    ...
    print("  --batch N: run every case with batch N")
...
def run_test(
    ...
    if isinstance(test_name_or_index_or_dict, dict):
        if "input" not in test_name_or_index_or_dict:
            print("Invalid test dict: must contain 'input' (static planner prompt string).")
            return None
...
def main(test: str = None, batch: int = None, no_thinking: bool = False):
    ...
    if batch is not None:
        ...
        if not idxs:
            print(f"No tests found for batch {batch}.")
            print_available_test_cases()
            return
    ...
    print("  Run one batch: --batch <N>  (see TEST_CASES batch field)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run planner optimizer v5 tests (P:AgentPlanner)")
    parser.add_argument("--test", type=str, help='Test name or index (e.g. "hows_my_accounts_doing" or "0"), or "all"')
    parser.add_argument("--batch", type=int, help="Run all cases with this batch number (1–6)")
