...
4.  **Reminder Creation**:
    ...
    -   **Specific vs Generic**: If request mentions specific entities (e.g. "Netflix", "Chase **1234"), they MUST exist in **Input Info**. If missing, return `(False, "Missing info for [Item]")`.
    -   **Vague Requests**: If request is vague (e.g. "remind me later", "remind me"), return `(False, "Missing 'What' and 'When' for the reminder.")`.
    -   **Construct `what`**: Append ALL available details from **Input Info** (e.g. "Netflix (last amt: $9.99)", "Chase Checking **1234 (bal: $500)") to make the reminder self-contained.
    -   Call `create_reminder(what=..., when=...)`.
    -   **Handle Output**: `create_reminder` returns `(success, message)`. If `success` is True but `message` is `None` (meaning condition not currently met), use "Reminder created (condition not currently met)." as the result string.
5.  **Output**:
...
def main(batch: int = 1):
  ...
  if batch == 1:
    ...
    raise ValueError("batch must be 1, 2, 3, 4, or 5")
