...
from penny.tool_funcs.create_reminder import create_reminder as create_reminder_impl
import sandbox
...
- **For Reminder Creation Requests (e.g., "remind me to cancel X", "notify me when Y happens"):**
  - Always perform a `lookup_user_accounts_transactions_income_and_spending_patterns` first to gather relevant account, transaction, or subscription information needed for the reminder.
  - Then, use `create_reminder` to create the reminder, incorporating the `input_info` from the lookup to make the reminder specific and actionable.
...
**3. Avoid Unnecessary `create_reminder` Calls:**
- If the Last User Request is a question (e.g., "what are the steps to save money?"), avoid using `create_reminder`.
- Only use `create_reminder` if the user explicitly asks you to *create*, *set up*, *establish*, or *track* a reminder.
...
- `create_reminder(creation_request: str, input_info: str = None) -> tuple[bool, str]`
...
input: **Last User Request**: Create a reminder to cancel Spotify subscription at the end of this year (December 31st).
**Previous Conversation**:
User: How much am I spending on subscriptions?
Assistant: You're currently paying $9.99 for Spotify, $15.99 for Netflix, and $10.99 for Hulu.
output:
```python
def execute_plan() -> tuple[bool, str]:
    success, lookup_result = lookup_user_accounts_transactions_income_and_spending_patterns(
        lookup_request="Find Spotify subscription details including last transaction date, next payment date, and account information."
    )
    if not success:
        return False, lookup_result

    success, creation_result = create_reminder(
...
input: **Last User Request**: Notify me immediately when my checking account balance drops below $1000.
**Previous Conversation**:
User: What's my current checking account balance?
Assistant: Your Chase Total Checking **1563 has $4567 and your Chase Checking **3052 has $1202.
output:
```python
def execute_plan() -> tuple[bool, str]:
    success, lookup_result = lookup_user_accounts_transactions_income_and_spending_patterns(
        lookup_request="Get current checking account balances and account details."
    )
    if not success:
        return False, lookup_result
...
class ReminderAgentOptimizer:
  """Handles all Gemini API interactions for financial reminder creation and optimization"""
  
  def __init__(self, model_name="gemini-flash-lite-latest"):
    """Initialize the Gemini agent with API configuration for financial reminder creation"""
    # API Configuration
  ...
  def generate_response(self, last_user_request: str, previous_conversation: str) -> str:
    """
    Generate a response using Gemini API for financial reminder creation.
...
def create_reminder(creation_request: str = None, input_info: str = None, what: str = None, when: str = None, **kwargs) -> tuple[bool, str]:
    """
    Wrapper function that handles reminder creation.
    - If called with `what` and `when` (from P:CreateBudgetOrGoalOrReminder generated code), uses the actual implementation
    - If called with `creation_request` (from ReminderAgentOptimizer), uses CreateBudgetOrGoalOrReminder to generate code
    
    Args:
        creation_request: Natural language request for what reminder to create (for ReminderAgentOptimizer)
        input_info: Optional input from another skill function (for ReminderAgentOptimizer)
        what: What to be reminded about (for P:CreateBudgetOrGoalOrReminder)
        when: When the reminder will be relevant (for P:CreateBudgetOrGoalOrReminder)
        **kwargs: Additional arguments (user_id, etc.)
    
    Returns:
        tuple[bool, str]: (success, output_string)
    """
    # If called with what and when, use the actual implementation from penny.tool_funcs.create_reminder
    if what is not None and when is not None:
        return create_reminder_impl(what, when)
    
    # Otherwise, use CreateBudgetOrGoalOrReminder to generate code (for ReminderAgentOptimizer flow)
    if creation_request is None:
        return False, "create_reminder requires either (what, when) or (creation_request)"
    
    # Use the optimizer to generate code
    optimizer = CreateBudgetOrGoalOrReminder()
    generated_code = optimizer.generate_response(creation_request, input_info)
    
    # Execute the generated code in sandbox with the actual create_reminder implementation available
    user_id = kwargs.get('user_id', 1)
    try:
        # Provide the actual create_reminder implementation to the sandbox namespace
        # so that when CreateBudgetOrGoalOrReminder generates code calling create_reminder(what=..., when=...),
        # it uses the actual implementation
        additional_namespace = {
            'create_reminder': create_reminder_impl
        }
        success, output_string, logs, goals_list = sandbox.execute_agent_with_tools(generated_code, user_id, additional_namespace)
        # Ensure output_string is always a string, never None
        if output_string is None:
            output_string = "Reminder creation completed but no output message was returned."
...
def _run_test_with_logging(last_user_request: str, previous_conversation: str, optimizer: ReminderAgentOptimizer = None):
  """
  ...
  Args:
    last_user_request: The last user request as a string
    previous_conversation: The previous conversation as a string
    optimizer: Optional ReminderAgentOptimizer instance. If None, creates a new one.
    
  Returns:
    The generated response string
  """
  if optimizer is None:
    optimizer = ReminderAgentOptimizer()
...
output:"""
  ...
  if code:
    ...
    try:
      ...
      def wrapped_create_reminder(*args, **kwargs):
        print(f"\n[FUNCTION CALL] create_reminder")
        print(f"  args: {args}")
        print(f"  kwargs: {kwargs}")
        
        # Check if this is a call with what/when (from P:CreateBudgetOrGoalOrReminder generated code)
        # or creation_request (from ReminderAgentOptimizer generated code)
        if len(args) == 2 and 'what' not in kwargs and 'when' not in kwargs and 'creation_request' not in kwargs:
          # Called as create_reminder(what, when) - use actual implementation
          what, when = args
          result = create_reminder_impl(what, when)
        elif 'what' in kwargs and 'when' in kwargs:
          # Called as create_reminder(what=..., when=...) - use actual implementation
          result = create_reminder_impl(kwargs['what'], kwargs['when'])
        elif 'creation_request' in kwargs or (len(args) > 0 and 'creation_request' not in kwargs and 'what' not in kwargs):
          # Called with creation_request - use wrapper that calls CreateBudgetOrGoalOrReminder
          result = create_reminder(*args, **kwargs)
        else:
          # Default: try to use actual implementation with what/when if available
          if 'what' in kwargs or 'when' in kwargs:
            what = kwargs.get('what', args[0] if len(args) > 0 else '')
            when = kwargs.get('when', args[1] if len(args) > 1 else '')
            result = create_reminder_impl(what, when)
          else:
            # Fallback to wrapper
            result = create_reminder(*args, **kwargs)
      ...
      namespace = {
        ...
        'create_reminder': wrapped_create_reminder,
      }
...
TEST_CASES = [
  # Subscription cancellation reminders (date-based)
  {
    "name": "cancel_spotify_end_of_year",
    "last_user_request": "Create a reminder to cancel Spotify subscription at the end of this year (December 31st).",
    "previous_conversation": ""
  },
  {
    "name": "cancel_netflix_specific_date",
    "last_user_request": "Remind me to cancel Netflix on March 15th, 2026.",
    "previous_conversation": ""
  },
  {
    "name": "cancel_subscription_with_context",
    "last_user_request": "Cancel that subscription at the end of the year.",
    "previous_conversation": "User: How much am I spending on subscriptions?\nAssistant: You're currently paying $9.99 for Spotify, $15.99 for Netflix, and $10.99 for Hulu."
  },
  
  # Account balance alerts (conditional/immediate)
  {
    "name": "notify_low_balance",
    "last_user_request": "Notify me immediately when my checking account balance drops below $1000.",
    "previous_conversation": ""
  },
  {
    "name": "alert_high_credit_balance",
    "last_user_request": "Alert me if my credit card balance exceeds $5000.",
    "previous_conversation": ""
  },
  {
    "name": "warn_before_rent_due",
    "last_user_request": "Warn me 5 days before my rent is due if my checking account balance is less than $2000.",
    "previous_conversation": ""
  },
  
  # Transaction monitoring reminders (conditional/immediate)
  {
    "name": "notify_paycheck_arrival",
    "last_user_request": "Notify me immediately whenever my paycheck from work hits my account.",
    "previous_conversation": ""
  },
  {
    "name": "alert_refund_received",
    "last_user_request": "Tell me as soon as my Amazon refund comes in.",
    "previous_conversation": ""
  },
  {
    "name": "monitor_specific_transaction",
    "last_user_request": "Alert me when a transaction over $500 is posted to my savings account.",
    "previous_conversation": ""
  },
  
  # payment reminders (date-based)
  {
    "name": "remind_pay_bills",
    "last_user_request": "Remind me to pay my credit card bill 3 days before it's due.",
    "previous_conversation": ""
  },
  {
    "name": "remind_tax_preparation",
    "last_user_request": "Create a reminder to prepare my taxes by January 31st, 2026.",
    "previous_conversation": ""
  },
  
  # Threshold and spending alerts (conditional)
  {
    "name": "alert_grocery_spending",
    "last_user_request": "Alert me when my grocery spending exceeds $400 this month.",
    "previous_conversation": ""
  },
  {
    "name": "notify_dining_out_limit",
    "last_user_request": "Notify me if I spend more than $300 on dining out this month.",
    "previous_conversation": ""
  },
  {
    "name": "alert_category_threshold",
    "last_user_request": "Alert me when my shopping category spending goes over $500 in a month.",
    "previous_conversation": ""
  },
  
  # Recurring and frequency-based reminders
  {
    "name": "remind_monthly_review",
    "last_user_request": "Remind me to review my budget at the end of each month.",
    "previous_conversation": ""
  },
  {
    "name": "notify_whenever_subscription_charges",
    "last_user_request": "Notify me whenever any subscription charges my account.",
    "previous_conversation": ""
  },
  
  # Complex scenarios requiring lookup + strategize
  {
    "name": "remind_based_on_analysis",
    "last_user_request": "Based on my spending patterns, remind me when I should review my subscriptions.",
    "previous_conversation": ""
  },
  ...
  # Multiple reminders in one request
  {
    ...
    "last_user_request": "Create a reminder to cancel Netflix at year end and alert me if my balance drops below $1000.",
    "previous_conversation": ""
  },
  
  # Edge cases
  {
    "name": "vague_reminder_request",
    "last_user_request": "Remind me about something later.",
    "previous_conversation": ""
  },
  {
    "name": "reminder_without_when",
    "last_user_request": "Create a reminder to cancel my subscription.",
    "previous_conversation": ""
  },
]
...
def run_test(test_name_or_index_or_dict, optimizer: ReminderAgentOptimizer = None):
  """
  Run a single test by name, index, or by passing test data directly.
  
  Args:
    test_name_or_index_or_dict: One of:
      - Test case name (str): e.g., "cancel_spotify_end_of_year"
...
def run_tests(test_names_or_indices_or_dicts, optimizer: ReminderAgentOptimizer = None):
  """
...
def test_with_inputs(last_user_request: str, previous_conversation: str, optimizer: ReminderAgentOptimizer = None):
  """
  Convenient method to test the reminder agent optimizer with custom inputs.
...
def main():
  """Main function to test the reminder agent optimizer"""
  # Option 1: Run a single test by name
  # run_test("cancel_spotify_end_of_year")
  
  # Option 2: Run a single test by index
  # run_test(0)  # cancel_spotify_end_of_year
  
  # Option 3: Run a single test by passing test data directly
  run_test({
    "name": "custom_test",
    # "last_user_request": "Create a reminder to cancel Netflix subscription at the end of this year.",
    "last_user_request": "Notify me whenever any subscription charges my account",
    "previous_conversation": ""
  })
  
  # Option 4: Run multiple tests by names
  # run_tests(["cancel_spotify_end_of_year", "notify_low_balance"])
  
  # Option 5: Run multiple tests by indices
  # run_tests([0, 1])  # cancel_spotify_end_of_year, notify_low_balance
