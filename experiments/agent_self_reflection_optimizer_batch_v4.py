...
class SelfReflectionWoCodeOptimizer:
  ...
  def generate_response(
    ...
    if prompt_override is not None:
      request_body = prompt_override
    else:
      request_body = _format_reflection_user_prompt(task_description, previous_outcomes, execution_result)
    request_text = types.Part.from_text(text=request_body)
...
def _format_reflection_user_prompt(
  task_description: str,
  previous_outcomes: dict[int | str, str] | list[str] | str | None,
  execution_result: str,
) -> str:
...
def _run_test_with_logging(
  ...
  if prompt_override is not None:
    ...
    llm_input_display = _format_reflection_user_prompt(task_description, previous_outcomes, execution_result)
  print(f"## LLM Input\n")
...
# Same salary/shelter sequences as strategizer batches. `input` pieces concatenate to the reflection user
# message: # Task Description, optional ## Previous Outcomes, ## Execution Result. `output` is unused (None).
TEST_CASES = [
  {
    "batch": 1,
  ...
  },
  {
    ...
    "input": (
      ...
      '\n\n'
      '## Previous Outcomes\n\n'
      '1. **Outcome #1**: The execution successfully listed recent income transactions, confirming the presence of two deposits ($2500 from ADP PAYROLL and $2500 from Gusto) that appear to be salary but are currently uncategorized. The task requires fixing the categorization if they are salary. Since the categorization fix has not yet occurred, the status must remain IN_PROGRESS to pursue the required correction.'
      '\n\n'
      '## Execution Result\n\n'
      "(True, 'Successfully processed categorization request: Categorize all transactions originating from 'ADP PAYROLL' and 'Gusto' as 'Salary'.')"
      '\n'
    ),
  ...
  },
  {
    ...
    "input": (
      ...
      '\n\n'
      '## Execution Result\n\n'
      "(True, 'An unexpected error was encountered.')"
      '\n'
    ),
  ...
  },
  {
    ...
    "input": (
      ...
      '\n\n'
      '## Previous Outcomes\n\n'
      '1. **Outcome #1**: The execution successfully listed recent income transactions, confirming the presence of two deposits ($2500 from ADP PAYROLL and $2500 from Gusto) that appear to be salary but are currently uncategorized. The task requires fixing the categorization if they are salary. Since the categorization fix has not yet occurred, the status must remain IN_PROGRESS to pursue the required correction.\n2. **Outcome #2**: The previous step successfully identified two potential salary transactions ($2500 from ADP PAYROLL and $2500 from Gusto) that needed categorization. However, the current execution step failed with a generic error message, preventing any further progress towards fixing the categorization or confirming the salary expectation. Therefore, the status is FAILED.'
      '\n\n'
      '## Execution Result\n\n'
      "(True, 'Successfully processed categorization request: Update transactions from source 'ADP PAYROLL' to category 'Salary'. Also, update transactions from source 'Gusto' to category 'Salary'.')"
      '\n'
    ),
  ...
  },
  {
    ...
    "input": (
      '# Task Description\n\n'
      "Look into the user's rent or mortgage spending and make sure it's categorized correctly as shelter_home. Check if amount is expected, and if it isn't transactions might be in uncategorized. Look at the largest spending to see if it is indeed rent or mortgage and fix the categorization."
      '\n\n'
      '## Execution Result\n\n'
  ...
  },
  {
    ...
    "input": (
      ...
      '\n\n'
      '## Previous Outcomes\n\n'
      "1. **Outcome #1**: The task requires checking amounts and fixing categorizations for rent/mortgage spending. The execution result successfully listed the top spending and revealed miscategorizations: a $2000 payment to Apartments LLC on 2025-11-18 is incorrectly labeled 'meals_dining_out', and a $1650 payment to Bank of America on 2025-10-31 is 'uncategorized'. Since these required fixes have not yet been applied, the workflow must continue to the remediation step. Rubric 0 does not apply as there are no two differing large housing payments on the same date."
      '\n\n'
      '## Execution Result\n\n'
      "(True, 'Successfully processed categorization request: Categorize transactions of $2000 to 'Apartments LLC' on 2025-11-18 with category meals_dining_out to shelter_home and of $1650 to 'Bank of America' on 2025-10-31 with category uncategorized to shelter_home as 'shelter_home'.')"
      '\n'
    ),
  ...
  },
  {
    ...
    "input": (
      ...
      '\n\n'
      '## Execution Result\n\n'
      "(True, 'Successfully processed categorization request: Categorize all transactions originating from 'Apartments LLC' as 'shelter_home'.')"
      '\n'
    ),
  ...
  },
  {
    ...
    "input": (
      ...
      '\n\n'
      '## Previous Outcomes\n\n'
      "1. **Outcome #1**: The task requires checking amounts and fixing categorizations for rent/mortgage spending. The execution result successfully listed the top spending and revealed miscategorizations: a $2000 payment to Apartments LLC on 2025-11-18 is incorrectly labeled 'meals_dining_out', and a $1650 payment to Bank of America on 2025-10-31 is 'uncategorized'. Since these required fixes have not yet been applied, the workflow must continue to the remediation step. Rubric 0 does not apply as there are no two differing large housing payments on the same date.\n2. **Outcome #2**: The execution result indicates an unexpected error occurred during the last step, despite previous steps identifying a clear miscategorization that needed fixing. Since the error prevents the necessary correction, the task cannot proceed successfully."
      '\n\n'
      '## Execution Result\n\n'
      "(True, 'Successfully processed categorization request: Categorize transactions of $2000 to 'Apartments LLC' on 2025-11-18 with category meals_dining_out to shelter_home and of $1650 to 'Bank of America' on 2025-10-31 with category uncategorized to shelter_home as 'shelter_home'.')"
      '\n'
