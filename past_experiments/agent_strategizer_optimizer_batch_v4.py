...
class StrategizerOptimizer:
  ...
  def generate_response(
    ...
    if prompt_override is not None:
      ...
      body = _format_strategizer_prompt(
        task_description,
        previous_outcomes,
...
def _format_strategizer_prompt(
  task_description: str,
  previous_outcomes: dict[int | str, str] | list[str] | str | None,
  latest_result_summary: str | None = None,
  latest_outcome_reflection: str | None = None,
...
def _run_test_with_logging(
  ...
  if prompt_override is not None:
    ...
    llm_input_display = _format_strategizer_prompt(
      task_description,
      previous_outcomes,
      latest_result_summary=latest_result_summary,
      latest_outcome_reflection=latest_outcome_reflection,
...
TEST_CASES = [
  {
    ...
    "output": "Expected: lookup finding total income and whether there are salary categories already mapped.",
  ...
  },
  {
    ...
    "output": "Expected: lookup finding total income and whether there are only uncategorized income transactions.",
  ...
  },
  {
    ...
    "input": (
      ...
      "Check if there's salary detected for this user and whether it looks as expected. If we can't find it, look at the list of amounts coming in and check if there are salary transactions mixed up over there. Fix the categorization of these if they are indeed Salary.\n\n"
      "## Previous Outcomes\n\n"
      "1. **Outcome #1**: The execution successfully listed recent income transactions, confirming the presence of two deposits ($2500 from ADP PAYROLL, transaction_id 50111; $2500 from Gusto, transaction_id 50112) that appear to be salary but are currently uncategorized. The task requires fixing the categorization if they are salary. Since the categorization fix has not yet occurred, the status must remain IN_PROGRESS to pursue the required correction.\n\n"
      "## Most Recent Attempt Result\n\n"
      "Two income transactions for $2500 each were found from ADP PAYROLL and Gusto, both categorized as 'uncategorized'.\n\n"
      "## Outcome & What to do Next\n\n"
      "Prior outcome already identified transaction_id 50111 (ADP PAYROLL) and 50112 (Gusto) as uncategorized salary-like deposits; recategorize as Salary (rules as appropriate)."
    ),
    "output": "Expected: recategorize ADP PAYROLL and Gusto uncategorized income transactions as Salary (and create/update rule if appropriate).",
  ...
  },
  {
    ...
    "input": (
      ...
      "1. **Outcome #1**:  The execution successfully listed recent income transactions, confirming the presence of two deposits ($2500 from ADP PAYROLL, transaction_id 50111; $2500 from Gusto, transaction_id 50112) that appear to be salary but are currently uncategorized. The task requires fixing the categorization if they are salary. Since the categorization fix has not yet occurred, the status must remain IN_PROGRESS to pursue the required correction.\n\n"
      "## Most Recent Attempt Result\n\n"
      "The execution returned an explicit failure message: 'An unexpected error was encountered.'\n\n"
      "## Outcome & What to do Next\n\n"
      "Prior outcome already identified transaction_id 50111 (ADP PAYROLL) and 50112 (Gusto) as uncategorized salary-like deposits; recategorize as Salary (rules as appropriate)."
    ),
    "output": "Expected: recategorize ADP PAYROLL and Gusto uncategorized income transactions as Salary (and create/update rule if appropriate).",
  ...
  # shelter (batch 2)
  {
    ...
    "output": "Expected: lookup rent or mortgage spending and whether they're categorized correctly as shelter_home.",
